...
import { PeriodConfig, MetricFilter, getPeriodConfig, calculateMetrics } from '@/lib/metricsCalculations';
  ...
  return <div className="min-h-screen bg-background pb-24">
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="space-y-4">
          ...
          {/* Grade trajectory */}
          <TrajectoryChart data={metrics.totalTrajectories.grade} title="Calificación del periodo" showVariation color="hsl(var(--primary))" />
          
          {/* Buenas obras trajectory */}
          <TrajectoryChart data={metrics.totalTrajectories.buenasObras} title="Buenas obras" color="hsl(142, 70%, 45%)" />
          
          {/* Mortal sins trajectory */}
          <TrajectoryChart data={metrics.totalTrajectories.mortalSins} title="Pecados mortales" color="hsl(0, 70%, 50%)" />
          
          {/* Venial sins trajectory */}
          <TrajectoryChart data={metrics.totalTrajectories.venialSins} title="Pecados veniales" color="hsl(45, 70%, 50%)" />
        </div>
        ...
        {/* Filtered Trajectory */}
        {hasFilters && metrics.filteredTrajectory && <div className="space-y-2">
            <h2 className="text-ios-headline font-semibold text-foreground">
              Trayectoria filtrada
            </h2>
            <TrajectoryChart data={metrics.filteredTrajectory} title="Filtro aplicado" color="hsl(280, 70%, 50%)" />
          </div>}
        ...
        {/* Notes panel */}
        {metrics.notesInPeriod.length > 0 && <div className="space-y-3">
            <h2 className="text-ios-headline font-semibold text-foreground">
              Notas del periodo
            </h2>
            <div className="bg-card rounded-xl border border-border divide-y divide-border">
              {metrics.notesInPeriod.map((note, idx) => <div key={idx} className="p-4">
